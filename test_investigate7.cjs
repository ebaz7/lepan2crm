...
async function main() {
  console.log("--- All rows for POY 160/48 سفید in OpCode 12 with Amt > 0 ---");
  ...
  `);
  console.log(rows);
  const totalQty = rows.reduce((s, r) => s + parseFloat(r.Qty), 0);
  const totalAmt = rows.reduce((s, r) => s + parseFloat(r.Amt), 0);
  console.log(`TOTAL Qty: ${totalQty}, TOTAL Amt: ${totalAmt}`);
}
